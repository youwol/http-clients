...
import { GetUserInfoResponse, QueryGroupsResponse } from '../../lib/assets-gateway'
import '../common'
...
describe('session', () => {
    test('backward compatible with assets-gateway/user-info', () => {
        return expect(
            ...
            name: 'John Doe',
            groups: [
                { 'id': 'private_51c42384-3582-494f-8c56-7405b01646ad', 'path': 'private' },
    ...
    test('backward compatible with assets-gateway/groups', () => {
        return expect(
            subject.getSessionDetails$()
                .pipe(raiseHTTPErrors()).toPromise()
                .then(session => ({ groups: session.userInfo.groups }) as QueryGroupsResponse),
