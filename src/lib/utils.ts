...
export function skipHTTPError<T>() : (src:Observable<T| HTTPError>) => Observable<T> {
...
export function nothing<T>() : (src:Observable<T| HTTPError>) => Observable<T| HTTPError> {
    return (source$: Observable<T | HTTPError>) => source$
}
...
export function requestToJson$<T = unknown>(request) {
